...
import heapq
from search_algorithms.a_star import a_star_3


class Agent:
    def __init__(self, start: Tuple[int, int], goal: Tuple[int, int], fuel: int):
        self.start = start
        self.goal = goal
        self.fuel = fuel
        self.path = []
    ...
    # Support < operator for heapq
    def __lt__(self, other):
        return True

    def __str__(self):
        return f"Agent: {self.start} -> {self.goal} with {self.fuel} fuel"
...
def get_agents(city_map: CityMap) -> List[Agent]:
    ...
    for i in range(len(city_map.start_points)):
        ...
        fuel = city_map.fuel_capacity
        agents.append(Agent(start, goal, fuel))
    return agents
...
def a_star_multi_agent(
    ...
    for agent in agents:
        ...
        print(agent)
        path = a_star_3(city_map, agent.start, agent.goal, agent.fuel, level=3)
        print(path)
        paths[agent] = path
